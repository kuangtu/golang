...
类型定义：两者是不同的类型，类型别名：两者是相同的类型。如下示例：
...
变量a和b的类型是不同的，需要强制转换。而d和e则是相同的类型。



## 3.4 字符串

一个字符串是**一个不可改变**的字节序列。字符串可以包含任意的数据，包括byte值0，但是通常是用来包含人类可读的文本。文本字符串通常被解释为采用UTF8编码的Unicode码点（rune）序列。
